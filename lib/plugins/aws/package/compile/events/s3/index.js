  ...
  newS3Buckets() {
    ...
    this.serverless.service.getAllFunctions().forEach((functionName) => {
      ...
      if (functionObj.events) {
        functionObj.events.forEach(event => { // eslint-disable-line consistent-return
      ...
      if (functionObj.events) {
        functionObj.events.forEach((event, idx) => {
          if (event.s3 && _.isObject(event.s3) && event.s3.existing) {
            ...
            const customResourceFunctionLogicalId = this.provider.naming
              .getCustomResourceS3HandlerFunctionLogicalId();
            const customS3ResourceLogicalId = this.provider.naming
              .getCustomResourceS3ResourceLogicalId(functionName, index);
